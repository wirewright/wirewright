module Ww
  struct Term
    ...
    # - `(held@ pattern)`: if the matchee is held applies `pattern`
    #   on the held value; otherwise, applies pattern to matchee directly.
    ...
    private def self.unify?(env, a : Sym, b : ITerm) : Dict?
      unless blank = a.polyblank? || a.blank?
    ...
    private def self.unify?(env, pattern : Items, matchee : Items, a : Sym) : {Dict, Items, Items}?
      ...
      polyblanks, _ = pattern.thru(Sym, &.polyblank?)
      unless polyblanks
      ...
      # string - not string edge (e.g. `xs_string+ ys+`), any - string edge
      # (`xs+ ys_string+`) and so on.
    ...
    #   (e.g. `match?({:foo, :"xs+"}, {:foo, 1, 2, 3})` results in the assignment `{ xs: {1, 2, 3} }`)
    ...
    #   (e.g. `match?({:"l+", :"r+"}, {1, 2, 3, 4})` results in the assignment `{ l: {1, 2}, r: {3, 4} }`.
    def self.match?(pattern, matchee) : Dict?
      unify?(Term[], Term[pattern].downcast, Term[matchee].downcast)
    end
  end
end
