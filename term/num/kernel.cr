module Ww
  ...
  struct Term::Num::Kernel
    ...
    def self.from(object : Int32)
      # Not sure if the mask is needed here, but without it we seem to get junk
      # in the high bits... which appears to gently hint us that using unsafe_as
      # here is a blunder?
      value = object.unsafe_as(UInt64) & 0xffffffffu64

      new(Pointer(Void).new((value << 3) | Tag::NumInt.value))
    end
    ...
    def self.pow(a : Kernel, b : Kernel)
      if a.int? && b.int? && !b.unsafe_as_i.negative?
        begin
          return from(a.unsafe_as_i ** b.unsafe_as_i)
        rescue OverflowError
        end
