module Ww::M1
  ...
  class PatternSet(T)
    ...
    # end
    # ```
    def self.select(selector : Term, bases : Enumerable(Term), *, key keymod : Key(T) = Key::Head, phase : Term? = nil, & : M1::Normp, Term::Dict -> Bool?) : PatternSet(T) forall T
