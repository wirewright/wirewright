...
module Ww::Oklch
  ...
  # Returns linear (0-1) R, G, B for the given OkLCH color.
  def to_lrgb(l : Float64, c : Float64, h : Float64, *, fit : FitMethod = :smart) : {Float64, Float64, Float64}
    l = l.clamp(0.0..1.0)
    c = c.clamp(0.0..0.36)
    h = h.clamp(0.0..360)
  ...
  #
  # - *l* (lightness) is in percents, clamped to 0-1.
  ...
  #
  # See `to_lrgb` to learn about *kwargs*.
  def to_rgb(l : Float64, c : Float64, h : Float64, **kwargs) : {UInt8, UInt8, UInt8}
    r, g, b = to_lrgb(l, c, h, **kwargs)
