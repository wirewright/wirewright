module Ww
  ...
  module Charset(N2)
    ...
    def compile(set : String, & : UInt32[N2]* -> T) : T forall T
      b = e = '\0'
      state0 = state1 = :initial

      mem = uninitialized UInt32[N2]
      cursor = 2u32
      negbegin = nil
      ...
      loop do
        ...
        when {:commit, _}
          ...
          if b == e
            ...
            mem[cursor] = (start.to_u32 << 1) | 1
            cursor += 1
          else
            if cursor + 2 > N2
              raise ArgumentError.new("charset `#{set}` exceeds memory limit for ranges (#{N2})")
            end
            mem[cursor] = start.to_u32 << 1
            mem[cursor + 1] = e.ord.to_u32
            cursor += 2
          end
    ...
    def match?(charset : UInt32[M]*, ch : Char) : Bool forall M
