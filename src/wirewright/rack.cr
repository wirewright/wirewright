module Ww::Rack
  ...
  def visualize(circuit : Term, blacklist : Set(Term) = Set{Term.of(:rule), Term.of(:backmap)}) : Term
    D7.image(clf.call(circuit), circuit) do |addr, feature|
      case feature
      in D7::ParentImage
        Term.case(feature.parent.node) do
          matchpi %{{¦ style}} { Slice[Term.of(feature.node)] }
      ...
      in D7::Gnd
        Term.case(feature.node) do
          matchpi %{{¦ style}} { Slice[feature.node] }
